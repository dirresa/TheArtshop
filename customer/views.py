...
def dang_nhap(request):
    ...
    if 's_khachhang' in request.session:
        return redirect('artshop:trang_chu')
    ...
    if request.POST.get('btnDangKy'):
        ...
        if form.is_valid():
            if form.cleaned_data['password'] == form.cleaned_data['confirm_password']:
                ...
                # + ', ' + form.cleaned_data['ward'] + ', ' + form.cleaned_data['district'] + ', ' + form.cleaned_data['province']
                post.save()
            ...
            ket_qua_dang_ky ='''
            <div class="alert alert-danger" role="alert">
                Registering faied! Please check your information again!
            </div>
            '''
    ...
    if request.POST.get('btnDangNhap'):
        ...
        mat_khau = hasher.encode(request.POST.get('mat_khau'), salt)
         ...
         # SELECT * FORM  customer_customer  WHERE email='' AND password=''
        ...
        if khach_hang.count() > 0:
            ...
            return redirect('artshop:trang_chu')
            # print(dict_khach_hang)
        else:
            ket_qua_dang_ky ='''
            <div class="alert alert-danger" role="alert">
                Logging in failed! Please check again!
            </div>
            '''

    return render(request, 'artshop/login.html', {
        'form': form, 
...
def tai_khoan_cua_toi(request):
    ...
    if request.POST.get('btnDoiMatKhau'):
        ...
        if mat_khau_hien_tai == khach_hang.password:
            ...
            ket_qua_doi_mat_khau = '''
            <div class="alert alert-danger" role="alert">
                You current password is incorrect. Check again!
            </div>
            '''
    ...
    return render(request, 'artshop/my-account.html', {
        'cart': cart,
...
def xuat_bao_cao_don_hang(request, pk):
    ...
    filename = 'ORDER' + str(order.pk) + '.pdf'
    folder_reports = settings.MEDIA_ROOT + 'artshop/reports/'
    path_to_report = folder_reports + filename
    pdfkit.from_string(response, path_to_report, configuration=config)
    return redirect('/media/artshop/reports/' + filename)
